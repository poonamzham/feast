...
from feast.infra.offline_stores.contrib.db2_offline_store.db2 import (
    DB2OfflineStoreConfig,
    DB2Source,
)
from feast.infra.utils.db2.connection_utils import df_to_db2_table
...
def db2_container():
    ...
    logger.info("Waited for %s seconds until db2 container was up", waited)

    yield container
    container.stop()


class DB2DataSourceCreator(DataSourceCreator, OnlineStoreCreator):
    def __init__(
        ...
        self.container = fixture_request.getfixturevalue("db2_container")
        if not self.container:
            raise RuntimeError(
                "In order to use this data source "
                "'feast.infra.offline_stores.contrib.db2_offline_store.tests' "
                "must be include into pytest plugins"
            )

        self.offline_store_config = DB2OfflineStoreConfig(
            type="db2",
            host="localhost",
    ...
    def create_data_source(
        ...
        if self.offline_store_config:
            df_to_db2_table(self.offline_store_config, df, destination_name)
        return DB2Source(
    ...
    def create_offline_store_config(self) -> DB2OfflineStoreConfig:
