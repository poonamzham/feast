...
from feast.infra.offline_stores.contrib.db2_offline_store.db2_source import (
    SavedDatasetDB2Storage,
)
...
from feast.infra.utils.db2.connection_utils import (
    _get_conn,
    df_to_db2_table,
    get_query_schema,
)
from feast.infra.utils.db2.db2_config import DB2Config
...
class DB2OfflineStoreConfig(DB2Config):
    type: Literal["db2"] = "db2"


class DB2OfflineStore(OfflineStore):
    @staticmethod
    @log_exceptions_and_usage(offline_store="db2")
    def pull_latest_from_table_or_query(
        ...
        assert isinstance(config.offline_store, DB2OfflineStoreConfig)
        assert isinstance(data_source, DB2Source)
    ...
    def get_historical_features(
        ...
        for fv in feature_views:
            assert isinstance(fv.batch_source, DB2Source)
...
class DB2RetrievalJob(RetrievalJob):
    def __init__(
        self,
    ...
    def persist(self, storage: SavedDatasetStorage, allow_overwrite: bool = False):
        assert isinstance(storage, SavedDatasetDB2Storage)

        df_to_db2_table(
            config=self.config.offline_store,
            df=self.to_df(),
            table_name=storage.db2_options._table,
        )
...
def build_point_in_time_query(
    ...
    """Build point-in-time query between each feature view table and the entity dataframe for DB2
    """
...
def _upload_entity_df(
    ...
    if isinstance(entity_df, pd.DataFrame):
        ...
        df_to_db2_table(config.offline_store, entity_df, table_name)
    elif isinstance(entity_df, str):
